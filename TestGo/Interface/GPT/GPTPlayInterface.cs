...
namespace MUCGO_zero_CS
{
    ...
    public static class GTPPlayInterface
    {
        ...
        private static readonly Dictionary<string, uint> GtpCmdSubId = new Dictionary<string, uint>() {
        ...
        private static readonly Dictionary<string, uint> GtpCmdArgc = new Dictionary<string, uint>() {
            { "protocol_version", 0U },
            { "boardsize", 1U },
            { "clear_board", 0U },
            { "name", 0U },
        ...
        private static readonly List<string> GtpMessage = new List<string>() {
            "",
        ...
        public static void ReadConfigs(GameConfig config)
        {
            Configs = config;
            //UCT搜索公共参数初始化
            ...
            PlayGame.Komi = (float)Configs.Komi;
            PlayGame.Init();
            PlayGame.UCT_Tree = new UCT_RL();
            PlayGame.UCT_Tree.NNInit();
        }
        // 参数分析（程序启动的时候需要查找的）
        public static void ParameterAnaylsis(string[] args)
        {
            Error.WriteLine(PROGRAM_NAME + " " + BoardInfo + " Initiated.");
            //PlayGame = new IGame();
                ...
                case 0x0U://genmove b/w
                    {
                        var arg = CmdStrides.Peek();
                        CmdStrides.Dequeue();
                        color = (arg == "b" || arg == "B") ? S_BLACK : ((arg == "w" || arg == "W") ? S_WHITE : S_OB);
                        break;
                    }
    ...
    class IGame
    {
        public bool RecordEnalbed = false;
        /*UCT搜索树*/
        public UCT_RL UCT_Tree = new UCT_RL();
        /// <summary>
        /// 投降标志(属性的定义)
        /// </summary>
        ...
        public int Genmove(Stone color)
        {
            ...
            int point = PASS;
            point = UCT_Tree.Genmove(Current);
            // 投降的时候不记录路径
            if (point != RESIGN)
            {
                PutStone(Current, (short)point, color);
                IsResigned = false;
            }
            else
            {
        ...
        {
            // 复制当前局面保存
            /*game_info_tnextsetp(new Board());
            CopyGame(nextsetp, GamePath.top());*/
            // 投降的时候不记录路径
            if (pos != RESIGN)
            {
                PutStone(Current, (short)pos, color);
        ...
        {
            // 自对弈强化学习
            UCT_Tree.ReinforceLearningGTP(PlayTime, TimeType.hour);
        }
        // 获得分数
        public float GetScore()
        {
            UCT_Tree.Analysis(Current);
        ...
        {
            // TODO: 在此处添加实现代码.
            if (pure_board_size != size && size <= EngineBoardSize && size > 0)
                SetBoardSize(size);
            Reset();
        }
        // 搜索模式使用定时搜索
        ...
        //读写让目(属性的定义) 
        public float Komi { get { return komi[0]; } set { SetKomi(value); } }
    };
}
